...
async def on_message(message):
    ...
    if message.content.lower().startswith('d!ping'):
        timep = time.time()
        emb = discord.Embed(title = 'Aguarde...', color = 0xFF0000)
        pingm0 = await client.send_message(message.channel, embed=emb)
        ping = time.time() - timep
        pingm1 = discord.Embed(title = 'Pong!', description = ':ping_pong: Ping - %.01f segundos' % ping, color=0xFF0000)
    ...
    elif message.content.lower().startswith("d!ex") and message.author.id == "403345462876438548":
        try:
            await client.send_message(message.channel, str(eval(message.content[5:])))
